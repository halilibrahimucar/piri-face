...
class WebSocketHandler {
    static async create(config) {
        ...
        try {
            // Yeni API'de signed URL zorunlu, agent ID ile doğrudan bağlanma artık desteklenmiyor
            if (!config.signedUrl && !config.agentId) {
                throw new Error("Either signedUrl or agentId must be provided");
            }

            // Configure WebSocket connection
            let wsUrl;
            if (config.signedUrl) {
                // Signed URL kullan (tercih edilen yöntem)
                wsUrl = config.signedUrl;
                console.log("Connecting with signed URL");
            } else {
                // Fallback: agent ID ile bağlan (eski yöntem, artık çalışmayabilir)
                ...
                wsUrl = `${serverOrigin}${serverPath}${config.agentId}`;
                console.warn("Using agent ID directly. Consider using signed URL instead.");
            }
            ...
            // Sample rate'i metadata'dan al
            let sampleRate = 24000; // Default
            if (metadata.agent_output_audio_format) {
                const formatMatch = metadata.agent_output_audio_format.match(/(\d+)/);
                if (formatMatch) {
                    sampleRate = parseInt(formatMatch[1]);
                }
            } else if (metadata.sample_rate) {
    ...
    static async waitForMetadata(webSocket) {
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                reject(new Error("Timeout waiting for conversation metadata"));
            }, 10000);
            ...
            const messageHandler = (event) => {
                try {
                    ...
                    if (data.type === "conversation_initiation_metadata" || data.type === "metadata") {
                        clearTimeout(timeout);
                        webSocket.removeEventListener("message", messageHandler);
                        const metadata = data.conversation_initiation_metadata_event || data.metadata || data;
                        console.log("Conversation metadata received:", metadata);
                        resolve(metadata);
                    } else if (data.type === "error") {
                        clearTimeout(timeout);
                        webSocket.removeEventListener("message", messageHandler);
                        const errorMsg = data.error?.message || data.message || "WebSocket error";
                        console.error("WebSocket error:", errorMsg, data);
                        reject(new Error(errorMsg));
                    } else {
                        // İlk mesaj metadata değilse, logla ve devam et
                        console.log("Waiting for metadata, received:", data.type);
                    }
                } catch (parseError) {
...
class ConversationManager {
    ...
    handleWebSocketMessage = (event) => {
        try {
            ...
            // Event tipi kontrolü
            if (!data.type) {
                // Event tipi yoksa, muhtemelen binary data veya farklı format
                this.settings.onDebug({ raw: data });
                return;
            }
            ...
            switch (data.type) {
                ...
                case "tentative_agent_response_internal_event":
                ...
                case "metadata":
                    // Metadata zaten işlendi, görmezden gel
                    break;
                case "error":
                    this.handleError(data.error?.message || "WebSocket error", data);
                    break;
                default:
                    this.settings.onDebug(data);
            }
        } catch (error) {
            this.handleError("Failed to parse WebSocket message", { event, error });
        }
    };
    ...
    handleInterruption(data) {
        const interruptionEvent = data.interruption_event || data;
        if (interruptionEvent.event_id) {
            this.lastInterruptTime = interruptionEvent.event_id;
        }
        this.fadeOutAudio();
    }

    handleAgentResponse(data) {
        const responseEvent = data.agent_response_event || data;
        const message = responseEvent.agent_response || responseEvent.message || responseEvent.text;
        if (message) {
    ...
    handleUserTranscript(data) {
        const transcriptEvent = data.user_transcription_event || data;
        const transcript = transcriptEvent.user_transcript || transcriptEvent.transcript || transcriptEvent.text;
        if (transcript) {
            this.settings.onMessage({
                source: "user",
                message: transcript
    ...
    handleTentativeResponse(data) {
        const tentativeEvent = data.tentative_agent_response_internal_event || data;
        const response = tentativeEvent.tentative_agent_response || tentativeEvent.response || tentativeEvent.text;
        if (response) {
    ...
    handleIncomingAudio(data) {
        const audioEvent = data.audio_event || data;
        const eventId = audioEvent.event_id || audioEvent.id || 0;
        const audioBase64 = audioEvent.audio_base_64 || audioEvent.audio || audioEvent.data;
        
        if (audioBase64) {
            if (this.lastInterruptTime <= eventId) {
                this.processIncomingAudio(audioBase64);
    ...
    handlePing(data) {
        ...
        const eventId = pingEvent.event_id || pingEvent.id;
        if (eventId) {
    ...
    handleInputAudioMessage = (event) => {
        if (this.connectionStatus === "connected" && this.connection.socket.readyState === WebSocket.OPEN) {
            try {
                // Yeni API formatı: audio chunk'ı doğrudan base64 olarak gönder
                const audioChunk = event.data[0];
                if (audioChunk && audioChunk.buffer) {
                    const audioData = JSON.stringify({
                        type: "audio",
                        user_audio_chunk: arrayBufferToBase64(audioChunk.buffer)
                    });
                    this.connection.socket.send(audioData);
                }
            } catch (error) {
                console.error("Error sending audio data:", error);
                this.handleError("Failed to send audio data", error);
            }
        }
    };
